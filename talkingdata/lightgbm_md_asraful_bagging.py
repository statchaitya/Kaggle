# -*- coding: utf-8 -*-
"""
Created on Sun Apr 29 18:50:59 2018
...
# -*- coding: utf-8 -*-
"""
Created on Fri Apr 27 16:53:33 2018

@author: cgokh
"""

FILENO= 6 #To distinguish the output file name.
debug=False  #Whethere or not in debuging mode
import pandas as pd
import time
import numpy as np
...
def DO(frm,to,fileno):
    ...
    train_df = pd.read_csv("C:/Kaggle/talkingdata/train.csv", parse_dates=['click_time'], skiprows=range(1,frm), nrows=to-frm, dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'is_attributed'])

    print('loading test data...')
    if debug:
        test_df = pd.read_csv("C:/Kaggle/talkingdata/test.csv", nrows=100000, parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
    else:
        test_df = pd.read_csv("C:/Kaggle/talkingdata/test.csv", parse_dates=['click_time'], dtype=dtypes, usecols=['ip','app','device','os', 'channel', 'click_time', 'click_id'])
...
'''
For bag in bags
run bag 1 with chunk 1
generate output file
save output file
clear everything
run bag 2 with chunk 2
.
.
.
'''

''' bag 1 : from 0 to 25000000 '''
sub=DO(0, 25000000, 1)
